...
//   SITE_BASE    default "/" (must match vite.config.ts's VITE_BASE)
...
function build404Html() {
  const pathSegmentsToKeep = SITE_BASE.replace(/^\/+|\/+$/g, '').split('/').filter(Boolean).length;

  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Vespa UI</title>
    <script type="text/javascript">
      // Single Page Apps for GitHub Pages (MIT, https://github.com/rafgraph/spa-github-pages)
      //
      // Generated by scripts/generate-seo.mjs — pathSegmentsToKeep must match
      // vite.config.ts build "base" (0 for "/" org/custom domain, 1 for "/repo/").
...
function main() {
  ...
  writeFileSync(join(PUBLIC_DIR, '404.html'), build404Html());

  console.log(
    `Generated sitemap.xml, llms.txt, llms-full.txt, 404.html, and ${docs.length} raw doc mirror(s) under public/docs-raw/.`
  );
}
